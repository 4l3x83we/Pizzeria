            ...
            <div class="col-md-12">
                <div class="table-wrap p-2 rounded-3 shadow-lg bg-white">
                    ...
                    <div class="mt-3 text-bg-secondary p-2">
                        ...
                        <div class="d-flex flex-wrap mt-3">
                            @if($role->permissions)
                                @foreach($role->permissions as $role_permission)
                                    <form method="POST" action="{{ route('admin.roles.permissions.revoke', [$role->id, $role_permission->id]) }}" onsubmit="return confirm('{{ __('Are you sure?') }}');">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <div class="mt-3">
                            <form action="{{ route('admin.roles.permissions', $role->id) }}" method="POST" class="row g-3">
                                @csrf
                                <div class="col-md-12">
