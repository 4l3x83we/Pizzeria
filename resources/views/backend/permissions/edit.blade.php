            ...
            <div class="col-md-12">
                <div class="table-wrap p-2 rounded-3 shadow-lg bg-white">
                    <div class="d-flex justify-content-end mb-3">
                        <a href="{{ route('backend.permissions.index') }}" class="btn btn-success">{{ __('Back') }}</a>
                    </div>
                    <div class="text-bg-secondary p-2">
                        <form action="{{ route('backend.permissions.update', $permission->id) }}" method="POST" class="row g-3">
                            @csrf
                            @method('PATCH')
                            <div class="col-md-12">
                    ...
                    <div class="mt-3 text-bg-secondary p-2">
                        ...
                        <div class="d-flex flex-wrap mt-3">
                            @if($permission->roles)
                                @foreach($permission->roles as $permission_role)
                                    <form method="POST" action="{{ route('backend.permissions.roles.remove', [$permission->id, $permission_role->id]) }}" onsubmit="return confirm('{{ __('Are you sure?') }}');">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <div class="mt-3">
                            <form action="{{ route('backend.permissions.roles', $permission->id) }}" method="POST" class="row g-3">
                                @csrf
                                <div class="col-md-12">
