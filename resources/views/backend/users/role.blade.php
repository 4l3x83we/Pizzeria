            ...
            <div class="col-md-12">
                <div class="table-wrap p-2 rounded-3 shadow-lg bg-white">
                    <div class="d-flex justify-content-end mb-3">
                        <a href="{{ route('backend.users.index') }}" class="btn btn-success">{{ __('Back') }}</a>
                    </div>
                    ...
                    <div class="mt-3 text-bg-secondary p-2">
                        ...
                        <div class="d-flex flex-wrap mt-3">
                            @if($user->roles)
                                @foreach($user->roles as $user_role)
                                    <form method="POST" action="{{ route('backend.users.roles.remove', [$user->id, $user_role->id]) }}" onsubmit="return confirm('{{ __('Are you sure?') }}');">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <div class="mt-3">
                            <form action="{{ route('backend.users.roles', $user->id) }}" method="POST" class="row g-3">
                                @csrf
                                <div class="col-md-12">
                    ...
                    <div class="mt-3 text-bg-secondary p-2">
                        ...
                        <div class="d-flex flex-wrap mt-3">
                            @if($user->permissions)
                                @foreach($user->permissions as $user_permission)
                                    <form method="POST" action="{{ route('backend.users.permissions.revoke', [$user->id, $user_permission->id]) }}" onsubmit="return confirm('{{ __('Are you sure?') }}');">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <div class="mt-3">
                            <form action="{{ route('backend.users.permissions', $user->id) }}" method="POST" class="row g-3">
                                @csrf
                                <div class="col-md-12">
